...
class Predict:
            ...
            for headers, seqs, refs in self.test_loader:
                ...
                if output_bpp is None:
                    ...
                    if use_constraint:
                        ...
                        scs, preds, bps, pfs, bpps = self.model(
                            seqs,
    ...
    def run(self, args, conf=None):
        ...
        self.predict(
            output_bpseq=args.bpseq, output_bpp=args.bpp, result=args.result, use_constraint=args.use_constraint
        )
    ...
    def add_args(cls, parser):
        ...
        # input
        subparser.add_argument("input", type=str, help="FASTA-formatted file or list of BPseq files")
        ...
        subparser.add_argument("--param", type=str, default="", help="file name of trained parameters")
        subparser.add_argument("--use-constraint", default=False, action="store_true")
        subparser.add_argument(
            "--result", type=str, default=None, help="output the prediction accuracy if reference structures are given"
        )
        subparser.add_argument(
            "--bpseq", type=str, default=None, help="output the prediction with BPSEQ format to the specified directory"
        )
        subparser.add_argument(
            "--bpp",
            type=str,
        ...
        gparser.add_argument(
            "--filter-size", type=int, action="append", help="the length of each filter of CNN (default: 5)"
        )
        gparser.add_argument(
            "--pool-size", type=int, action="append", help="the width of the max-pooling layer of CNN (default: 1)"
        )
        gparser.add_argument("--dilation", type=int, default=0, help="Use the dilated convolution (default: 0)")
        gparser.add_argument(
            "--num-lstm-layers", type=int, default=0, help="the number of the LSTM hidden layers (default: 0)"
        )
        gparser.add_argument(
            "--num-lstm-units", type=int, default=0, help="the number of the LSTM hidden units (default: 0)"
        )
        gparser.add_argument(
            "--num-transformer-layers", type=int, default=0, help="the number of the transformer layers (default: 0)"
        )
        gparser.add_argument(
